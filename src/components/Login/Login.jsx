...
const Login = () => {
  ...
  return (
    <> 
     {!hasError && hasSubmitted && <div>
          <p data-testid="success" className="login__success">
            Thank you for submitting! We will be in touch
          </p>
        </div> }
      {!hasSubmitted && 
        <form data-testid="form" onSubmit={handleSubmit} name="form" className="login">
          ...
          <Button
            ...
            buttonText={"Login"}
          />
          {hasError && <div className="login__alert">
          <p>Sorry something went wrong </p> </div>}
        </form>}
